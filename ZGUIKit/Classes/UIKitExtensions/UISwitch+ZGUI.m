...
#import "UISwitch+ZGUI.h"
#import "ZGUICore.h"

@implementation UISwitch (ZGUI)
        ...
        ExtendImplementationOfNonVoidMethodWithSingleArgument([UISwitch class], @selector(initWithFrame:), CGRect, UISwitch *, ^UISwitch *(UISwitch *selfObject, CGRect firstArgv, UISwitch *originReturnValue) {
            if (ZGUICMIActivated) {
                ...
                if (SwitchOffTintColor) {
                    selfObject.zgui_offTintColor = SwitchOffTintColor;
                }
            }
            return originReturnValue;
        });
        
        // 设置 zgui_offTintColor 的原理是找到 UISwitch 内部的 switchWellView 并改变它的 backgroundColor，而 switchWellView 在某些时机会重新创建 ，因此需要在这些时机之后对 switchWellView 重新设置一次背景颜色：
            ...
            ExtendImplementationOfVoidMethodWithSingleArgument([UISwitch class], @selector(traitCollectionDidChange:), UITraitCollection *, ^(UISwitch *selfObject, UITraitCollection *previousTraitCollection) {
                ...
                if (interfaceStyleChanged) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [selfObject zgui_applyOffTintColorIfNeeded];
                    });
                }
            });
        } else {
...
- (void)setZgui_offTintColor:(UIColor *)zgui_offTintColor {
    UIView *switchWellView = [self valueForKeyPath:@"_visualElement._switchWellView"];
    UIColor *defaultOffTintColor = [switchWellView zgui_getBoundObjectForKey:kDefaultOffTintColorKey];
    if (!defaultOffTintColor) {
        defaultOffTintColor = switchWellView.backgroundColor;
        [switchWellView zgui_bindObject:defaultOffTintColor forKey:kDefaultOffTintColorKey];
    }
    // 当 offTintColor 为 nil 时，恢复默认颜色（和 setOnTintColor 行为保持一致）
    switchWellView.backgroundColor = zgui_offTintColor ? : defaultOffTintColor;
    objc_setAssociatedObject(self, &kAssociatedObjectKey_offTintColor, zgui_offTintColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIColor *)zgui_offTintColor {
    return objc_getAssociatedObject(self, &kAssociatedObjectKey_offTintColor);
}

- (void)zgui_applyOffTintColorIfNeeded {
    if (self.zgui_offTintColor) {
        self.zgui_offTintColor = self.zgui_offTintColor;
    }
}
