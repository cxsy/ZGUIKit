...
//  zgui
//
//  Created by ZGUI Team on 2017/2/14.
//

#import "UITabBar+ZGUI.h"
#import "ZGUICore.h"
...
@implementation UITabBar (ZGUI)

ZGUISynthesizeBOOLProperty(canItemRespondDoubleTouch, setCanItemRespondDoubleTouch)
ZGUISynthesizeNSIntegerProperty(lastTouchedTabBarItemViewIndex, setLastTouchedTabBarItemViewIndex)
ZGUISynthesizeNSIntegerProperty(tabBarItemViewTouchCount, setTabBarItemViewTouchCount)
    ...
    dispatch_once(&onceToken, ^{
        ...
        ExtendImplementationOfVoidMethodWithoutArguments([UITabBarController class], @selector(viewDidLoad), ^(UITabBarController *selfObject) {
            if (ZGUICMIActivated) {
                if (@available(iOS 13.0, *)) {
                    // iOS 13 不使用 tintColor 了，改为用 UITabBarAppearance，具体请看 ZGUIConfiguration.m
                } else {
        ...
        if (@available(iOS 13.0, *)) {
            ...
            ExtendImplementationOfVoidMethodWithoutArguments(NSClassFromString(@"_UITabBarVisualProviderLegacyIOS"), NSSelectorFromString(@"_updateBackground"), ^(NSObject *selfObject) {
                UITabBar *tabBar = [selfObject zgui_valueForKey:@"tabBar"];
                if (!tabBar) return;
                UIImage *shadowImage = tabBar.shadowImage;// 就算 tabBar 显示系统的分隔线，但依然能从 shadowImage 属性获取到业务自己设置的图片
                UIImageView *shadowImageView = tabBar.zgui_shadowImageView;
        ...
        OverrideImplementation([UITabBar class], @selector(setItems:animated:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^void(UITabBar *selfObject, NSArray<UITabBarItem *> *items, BOOL animated) {
                ...
                [items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
                    // 双击 tabBarItem 的功能需要在设置完 item 后才能获取到 zgui_view 来实现
                    UIControl *itemView = (UIControl *)item.zgui_view;
        ...
        OverrideImplementation([UITabBar class], @selector(setFrame:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITabBar *selfObject, CGRect frame) {
                if (ZGUICMIActivated && ShouldFixTabBarTransitionBugInIPhoneX && IOS_VERSION < 11.2 && IS_58INCH_SCREEN) {
        ...
        if (@available(iOS 13.0, *)) {
            ...
            UITabBarItem *(^tabBarItemOfLabelBlock)(UILabel *label) = ^UITabBarItem *(UILabel *label) {
                UIControl *tabBarButton = [label zgui_valueForKey:@"_tabBarButton"];
                UITabBar *tabBar = [tabBarButton zgui_valueForKey:@"tabBar"];
            ...
            OverrideImplementation(tabBarButtonLabelClass, NSSelectorFromString(@"_setUnselectedTintColor:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UILabel *selfObject, UIColor *firstArgv) {
                    ...
                    if (item) {
                        UITabBar *tabBar = [[selfObject zgui_valueForKey:@"_tabBarButton"] zgui_valueForKey:@"tabBar"];
                        NSDictionary<NSAttributedStringKey,id> *normalAttributes = [item titleTextAttributesForState:UIControlStateNormal] ?: [UITabBarItem.zgui_appearanceConfigured titleTextAttributesForState:UIControlStateNormal];
            ...
            OverrideImplementation(tabBarButtonLabelClass, @selector(setSelected:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UILabel *selfObject, BOOL selected) {
                    ...
                    NSDictionary<NSAttributedStringKey,id> *normalAttributes = [item titleTextAttributesForState:UIControlStateNormal] ?: [UITabBarItem.zgui_appearanceConfigured titleTextAttributesForState:UIControlStateNormal];
                    NSDictionary<NSAttributedStringKey,id> *selectedAttributes = [item titleTextAttributesForState:UIControlStateSelected] ?: [UITabBarItem.zgui_appearanceConfigured titleTextAttributesForState:UIControlStateSelected];
            ...
            OverrideImplementation(NSClassFromString(@"UITabBarButtonLabel"), @selector(sizeThatFits:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^CGSize(UILabel *selfObject, CGSize firstArgv) {
                    ...
                    if ([font respondsToSelector:selectorForCSSFontFamily]) {
                        BOOL forWebKit = YES;
                        NSString *fontFamily = [font zgui_performSelector:selectorForCSSFontFamily withArguments:&forWebKit, nil];
        ...
        if (@available(iOS 14.0, *)) {
            if (@available(iOS 14.2, *)) {
                ...
                OverrideImplementation([UINavigationController class], @selector(zgui_didInitialize), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                    return ^(UINavigationController *selfObject) {
                        ...
                        [selfObject zgui_addNavigationActionDidChangeBlock:^(ZGUINavigationAction action, BOOL animated, __kindof UINavigationController * _Nullable weakNavigationController, __kindof UIViewController * _Nullable appearingViewController, NSArray<__kindof UIViewController *> * _Nullable disappearingViewControllers) {
                            switch (action) {
                                case ZGUINavigationActionWillPop:
                                case ZGUINavigationActionWillSet: {
                                    ...
                                    if (animated && weakNavigationController.tabBarController && !appearingViewController.hidesBottomBarWhenPushed) {
                                        ...
                                        if (!systemShouldHideTabBar) {
                                            [weakNavigationController zgui_bindBOOL:YES forKey:kShouldCheckTabBarHiddenKey];
                                        }
                                    }
                                }
                                    break;
                                case ZGUINavigationActionDidPop:
                                case ZGUINavigationActionDidSet: {
                                    [weakNavigationController zgui_bindBOOL:NO forKey:kShouldCheckTabBarHiddenKey];
                                }
                                    break;
                ...
                OverrideImplementation([UINavigationController class], NSSelectorFromString(@"_shouldBottomBarBeHidden"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                    return ^BOOL(UINavigationController *selfObject) {
                        ...
                        if ([selfObject zgui_getBoundBOOLForKey:kShouldCheckTabBarHiddenKey]) {
                            result = NO;
                        }
                        return result;
                    };
        ...
        // 但这样有个风险，因为 ZGUIConfiguration 配置表里都是用 appearance 的方式去设置 standardAppearance，所以如果在 UITabBar 实例被添加到 window 之前修改过旧版任意一个样式接口，就会导致一个新的 UITabBarAppearance 对象被设置给 standardAppearance 属性，这样系统就会认为你这个 UITabBar 实例自定义了 standardAppearance，那么当它被 moveToWindow 时就不会自动应用 appearance 的值了，因此需要保证在添加到 window 前不要自行修改属性
        if (@available(iOS 13.0, *)) {
            ...
            void (^syncAppearance)(UITabBar *, void(^barActionBlock)(UITabBarAppearance *appearance), void (^itemActionBlock)(UITabBarItemAppearance *itemAppearance)) = ^void(UITabBar *tabBar, void(^barActionBlock)(UITabBarAppearance *appearance), void (^itemActionBlock)(UITabBarItemAppearance *itemAppearance)) {
                ...
                if (itemActionBlock) {
                    [appearance zgui_applyItemAppearanceWithBlock:itemActionBlock];
                }
...
- (UIView *)zgui_backgroundView {
    return [self zgui_valueForKey:@"_backgroundView"];
}

- (UIImageView *)zgui_shadowImageView {
    if (@available(iOS 13, *)) {
        return [self.zgui_backgroundView zgui_valueForKey:@"_shadowView1"];
    }
    // iOS 10 及以后，在 UITabBar 初始化之后就能获取到 backgroundView 和 shadowView 了
    return [self.zgui_backgroundView zgui_valueForKey:@"_shadowView"];
}
...
- (void)handleTabBarItemViewEvent:(UIControl *)itemView {
    ...
    if (!self.selectedItem.zgui_doubleTapBlock) {
        return;
    }
    ...
    if (self.tabBarItemViewTouchCount == 2) {
        ...
        if (item.zgui_doubleTapBlock) {
            item.zgui_doubleTapBlock(item, selectedIndex);
        }
        [self revertTabBarItemTouch];
    }
}
...
@implementation UITabBarAppearance (ZGUI)

- (void)zgui_applyItemAppearanceWithBlock:(void (^)(UITabBarItemAppearance * _Nonnull))block {
