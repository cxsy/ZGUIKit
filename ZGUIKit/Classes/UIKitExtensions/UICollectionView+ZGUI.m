...
//
//  UICollectionView+ZGUI.m
//  zgui
//
//  Created by ZGUI Team on 15/7/20.
//

#import "UICollectionView+ZGUI.h"
#import "ZGUICore.h"

@implementation UICollectionView (ZGUI)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UICollectionView class], @selector(scrollToItemAtIndexPath:atScrollPosition:animated:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UICollectionView *selfObject, NSIndexPath *indexPath, UICollectionViewScrollPosition scrollPosition, BOOL animated) {
                ...
                if (!isIndexPathLegal) {
//                    ZGUILogWarn(@"UICollectionView (ZGUI)", @"%@ - target indexPath : %@ ，不合法的indexPath。\n%@", selfObject, indexPath, [NSThread callStackSymbols]);
...
- (void)zgui_reloadDataKeepingSelection {
...
- (NSIndexPath *)zgui_indexPathForItemAtView:(id)sender {
...
- (BOOL)zgui_itemVisibleAtIndexPath:(NSIndexPath *)indexPath {
...
- (NSArray<NSIndexPath *> *)zgui_indexPathsForVisibleItems {
...
- (NSIndexPath *)zgui_indexPathForFirstVisibleCell {
    NSArray *visibleIndexPaths = [self zgui_indexPathsForVisibleItems];
