...
extern const CGFloat ZGUILineHeightIdentity;

@interface UILabel (ZGUI)

- (instancetype)zgui_initWithFont:(nullable UIFont *)font textColor:(nullable UIColor *)textColor;
 ...
 *
 */
@property(nullable, nonatomic, copy) NSDictionary<NSAttributedStringKey, id> *zgui_textAttributes;

/** 
 *  Setter 设置当前整段文字的行高
 *  @note 如果同时通过 zgui_textAttributes 或 attributedText 给整段文字设置了行高，则此方法将不再生效。换句话说，此方法设置的行高将永远不会覆盖 zgui_textAttributes 或 attributedText 设置的行高。
 *  @note 比如对于字符串"abc"，你通过 attributedText 设置 {0, 1} 这个 range 范围内的行高为 10，又通过 setZgui_lineHeight: 设置了整体行高为 20，则最终 {0, 1} 内的行高将为 10，而 {1, 2} 内的行高将为全局行高 20
 *  @note 比如对于字符串"abc"，你先通过 setZgui_lineHeight: 设置整体行高为 10，又通过 attributedText/zgui_textAttributes 设置整体行高为 20，无论这两个设置的代码的先后顺序如何，最终行高都将为 20
 *  @note 你可以通过设置 'ZGUILineHeightIdentity' 来恢复 UILabel 默认的行高
 *  @note 当你设置了此属性后，每次你调用 setText: 时，其实都会被自动转而调用 setAttributedText:
 *
 *  -----------------------------------
 *
 *  Getter 获取整段文字的行高
 *  @note 如果通过 setZgui_lineHeight 设置行高，会优先返回该值。
...
@property(nonatomic, assign) CGFloat zgui_lineHeight;
