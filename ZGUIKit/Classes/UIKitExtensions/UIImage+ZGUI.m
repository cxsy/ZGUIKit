...
//                    ZGUILogWarn(@"UIImage (ZGUI)", @"UIImage (ZGUI) resizableImageWithCapInsets 传进来的 capInsets 的水平/垂直方向的和应该小于图片本身的大小，否则会导致 render 时出现 invalid context 0x0 的错误。");
                }
...
+ (UIImage *)zgui_imageWithSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale actions:(void (^)(CGContextRef contextRef))actionBlock {
...
- (BOOL)zgui_resizable {
    BOOL result;
    [self zgui_performSelector:NSSelectorFromString(@"_isResizable") withPrimitiveReturnValue:&result];
    return result;
}

- (CGSize)zgui_sizeInPixel {
...
- (UIImage *)zgui_grayImage {
    // CGBitmapContextCreate 是无倍数的，所以要自己换算成1倍
    CGSize size = self.zgui_sizeInPixel;
    ...
    if (self.zgui_opaque) {
        ...
        // 用 CGBitmapContextCreateImage 方式创建出来的图片，CGImageAlphaInfo 总是为 CGImageAlphaInfoNone，导致 zgui_opaque 与原图不一致，所以这里再做多一步
        grayImage = [UIImage zgui_imageWithSize:grayImage.size opaque:NO scale:grayImage.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)zgui_imageWithAlpha:(CGFloat)alpha {
    return [UIImage zgui_imageWithSize:self.size opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size) blendMode:kCGBlendModeNormal alpha:alpha];
    }];
}

- (UIImage *)zgui_imageWithTintColor:(UIColor *)tintColor {
...
//    }
    BOOL opaque = self.zgui_opaque ? tintColor.zgui_alpha >= 1.0 : NO;// 如果图片不透明但 tintColor 半透明，则生成的图片也应该是半透明的
    return [UIImage zgui_imageWithSize:self.size opaque:opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)zgui_imageWithBlendColor:(UIColor *)blendColor {
    UIImage *coloredImage = [self zgui_imageWithTintColor:blendColor];
...
- (UIImage *)zgui_imageWithImageAbove:(UIImage *)image atPoint:(CGPoint)point {
    return [UIImage zgui_imageWithSize:self.size opaque:self.zgui_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size)];
        [image drawAtPoint:point];
    }];
}

- (UIImage *)zgui_imageWithSpacingExtensionInsets:(UIEdgeInsets)extension {
    ...
    return [UIImage zgui_imageWithSize:contextSize opaque:self.zgui_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawAtPoint:CGPointMake(extension.left, extension.top)];
    }];
}

- (UIImage *)zgui_imageWithClippedRect:(CGRect)rect {
...
- (UIImage *)zgui_imageWithClippedCornerRadius:(CGFloat)cornerRadius {
    return [self zgui_imageWithClippedCornerRadius:cornerRadius scale:self.scale];
}

- (UIImage *)zgui_imageWithClippedCornerRadius:(CGFloat)cornerRadius scale:(CGFloat)scale {
    if (cornerRadius <= 0) {
        return self;
    }
    return [UIImage zgui_imageWithSize:self.size opaque:NO scale:scale actions:^(CGContextRef contextRef) {
        [[UIBezierPath bezierPathWithRoundedRect:CGRectMakeWithSize(self.size) cornerRadius:cornerRadius] addClip];
        [self drawInRect:CGRectMakeWithSize(self.size)];
    }];
}

- (UIImage *)zgui_imageResizedInLimitedSize:(CGSize)size {
    return [self zgui_imageResizedInLimitedSize:size resizingMode:ZGUIImageResizingModeScaleAspectFit];
}

- (UIImage *)zgui_imageResizedInLimitedSize:(CGSize)size resizingMode:(ZGUIImageResizingMode)resizingMode {
    return [self zgui_imageResizedInLimitedSize:size resizingMode:resizingMode scale:self.scale];
}

- (UIImage *)zgui_imageResizedInLimitedSize:(CGSize)size resizingMode:(ZGUIImageResizingMode)resizingMode scale:(CGFloat)scale {
    ...
    if (resizingMode >= ZGUIImageResizingModeScaleAspectFit && resizingMode <= ZGUIImageResizingModeScaleAspectFillBottom) {
        CGFloat horizontalRatio = size.width / imageSize.width;
        CGFloat verticalRatio = size.height / imageSize.height;
        CGFloat ratio = 0;
        if (resizingMode >= ZGUIImageResizingModeScaleAspectFill && resizingMode < (ZGUIImageResizingModeScaleAspectFill + 10)) {
            ratio = MAX(horizontalRatio, verticalRatio);
        } else {
            // 默认按 ZGUIImageResizingModeScaleAspectFit
        ...
        // 默认按照 ZGUIImageResizingModeScaleToFill
        drawingRect = CGRectMakeWithSize(size);
        contextSize = size;
    }
    
    return [UIImage zgui_imageWithSize:contextSize opaque:self.zgui_opaque scale:scale actions:^(CGContextRef contextRef) {
        [self drawInRect:drawingRect];
    }];
}

- (UIImage *)zgui_imageWithOrientation:(UIImageOrientation)orientation {
    ...
    return [UIImage zgui_imageWithSize:contextSize opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)zgui_imageWithBorderColor:(UIColor *)borderColor path:(UIBezierPath *)path {
    if (!borderColor) {
        return self;
    }
...
- (UIImage *)zgui_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius {
    return [self zgui_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:cornerRadius dashedLengths:0];
}

- (UIImage *)zgui_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius dashedLengths:(const CGFloat *)dashedLengths {
    ...
    return [self zgui_imageWithBorderColor:borderColor path:path];
}

- (UIImage *)zgui_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth borderPosition:(ZGUIImageBorderPosition)borderPosition {
    if (borderPosition == ZGUIImageBorderPositionAll) {
        return [self zgui_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((ZGUIImageBorderPositionBottom & borderPosition) == ZGUIImageBorderPositionBottom) {
        ...
        if ((ZGUIImageBorderPositionTop & borderPosition) == ZGUIImageBorderPositionTop) {
            [path moveToPoint:CGPointMake(0, borderWidth / 2)];
            [path addLineToPoint:CGPointMake(self.size.width, borderWidth / 2)];
        }
        if ((ZGUIImageBorderPositionLeft & borderPosition) == ZGUIImageBorderPositionLeft) {
            [path moveToPoint:CGPointMake(borderWidth / 2, 0)];
            [path addLineToPoint:CGPointMake(borderWidth / 2, self.size.height)];
        }
        if ((ZGUIImageBorderPositionRight & borderPosition) == ZGUIImageBorderPositionRight) {
...
- (UIImage *)zgui_imageWithMaskImage:(UIImage *)maskImage usingMaskImageMode:(BOOL)usingMaskImageMode {
...
+ (UIImage *)zgui_animatedImageWithData:(NSData *)data {
    return [self zgui_animatedImageWithData:data scale:1];
}

+ (UIImage *)zgui_animatedImageWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self zgui_frameDurationAtIndex:i source:source];
...
+ (float)zgui_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)zgui_animatedImageNamed:(NSString *)name {
    return [UIImage zgui_animatedImageNamed:name scale:1];
}

+ (UIImage *)zgui_animatedImageNamed:(NSString *)name scale:(CGFloat)scale {
    ...
    return [UIImage zgui_animatedImageWithData:data scale:scale];
}

+ (UIImage *)zgui_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size path:(UIBezierPath *)path addClip:(BOOL)addClip {
    size = CGSizeFlatted(size);
    return [UIImage zgui_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)zgui_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth cornerRadius:(CGFloat)cornerRadius {
    ...
    return [UIImage zgui_imageWithStrokeColor:strokeColor size:size path:path addClip:NO];
}

+ (UIImage *)zgui_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth borderPosition:(ZGUIImageBorderPosition)borderPosition {
    CGContextInspectSize(size);
    if (borderPosition == ZGUIImageBorderPositionAll) {
        return [UIImage zgui_imageWithStrokeColor:strokeColor size:size lineWidth:lineWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((ZGUIImageBorderPositionBottom & borderPosition) == ZGUIImageBorderPositionBottom) {
...
+ (UIImage *)zgui_imageWithColor:(UIColor *)color {
    return [UIImage zgui_imageWithColor:color size:CGSizeMake(4, 4) cornerRadius:0];
}

+ (UIImage *)zgui_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadius:(CGFloat)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    
    color = color ? color : UIColorClear;
	BOOL opaque = (cornerRadius == 0.0 && [color zgui_alpha] == 1.0);
    return [UIImage zgui_imageWithSize:size opaque:opaque scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)zgui_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadiusArray:(NSArray<NSNumber *> *)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage zgui_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        UIBezierPath *path = [UIBezierPath zgui_bezierPathWithRoundedRect:CGRectMakeWithSize(size) cornerRadiusArray:cornerRadius lineWidth:0];
        [path addClip];
        [path fill];
    }];
}

+ (UIImage *)zgui_imageWithShape:(ZGUIImageShape)shape size:(CGSize)size lineWidth:(CGFloat)lineWidth tintColor:(UIColor *)tintColor {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage zgui_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        switch (shape) {
            ...
            case ZGUIImageShapeNavBack: {
                ...
                break;
            case ZGUIImageShapeDisclosureIndicator: {
                drawByStroke = YES;
                path = [UIBezierPath bezierPath];
                path.lineWidth = lineWidth;
        ...
        if (shape == ZGUIImageShapeDetailButtonImage) {
...
+ (UIImage *)zgui_imageWithShape:(ZGUIImageShape)shape size:(CGSize)size tintColor:(UIColor *)tintColor {
    CGFloat lineWidth = 0;
    switch (shape) {
        case ZGUIImageShapeNavBack:
            lineWidth = 2.0f;
    ...
    return [UIImage zgui_imageWithShape:shape size:size lineWidth:lineWidth tintColor:tintColor];
}

+ (UIImage *)zgui_imageWithAttributedString:(NSAttributedString *)attributedString {
    ...
    return [UIImage zgui_imageWithSize:stringSize opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [attributedString drawInRect:CGRectMakeWithSize(stringSize)];
    }];
}

+ (UIImage *)zgui_imageWithView:(UIView *)view {
    CGContextInspectSize(view.bounds.size);
    return [UIImage zgui_imageWithSize:view.bounds.size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [view.layer renderInContext:contextRef];
    }];
}

+ (UIImage *)zgui_imageWithView:(UIView *)view afterScreenUpdates:(BOOL)afterUpdates {
