...
@interface UITextView (ZGUI)

/**
 *  convert UITextRange to NSRange, for example, [self zgui_convertNSRangeFromUITextRange:self.markedTextRange]
 */
 ...
 */
- (nullable UITextRange *)zgui_convertUITextRangeFromNSRange:(NSRange)range;

/**
 *  设置 text 会让 selectedTextRange 跳到最后一个字符，导致在中间修改文字后光标会跳到末尾，所以设置前要保存一下，设置后恢复过来
 */
- (void)zgui_setTextKeepingSelectedRange:(NSString *)text;

/**
 *  设置 attributedText 会让 selectedTextRange 跳到最后一个字符，导致在中间修改文字后光标会跳到末尾，所以设置前要保存一下，设置后恢复过来
 */
- (void)zgui_setAttributedTextKeepingSelectedRange:(NSAttributedString *)attributedText;
 ...
 */
- (void)zgui_scrollRangeToVisible:(NSRange)range;

/**
 * 将光标滚到可视区域
 */
- (void)zgui_scrollCaretVisibleAnimated:(BOOL)animated;
