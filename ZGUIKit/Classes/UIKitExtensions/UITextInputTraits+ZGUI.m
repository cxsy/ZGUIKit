...
@property(nonatomic, assign) BOOL qti_setKeyboardAppearanceByZGUITheme;
@end

@implementation NSObject (ZGUITextInput)

ZGUISynthesizeBOOLProperty(qti_didInitialize, setQti_didInitialize)
ZGUISynthesizeBOOLProperty(qti_setKeyboardAppearanceByZGUITheme, setQti_setKeyboardAppearanceByZGUITheme)
        ...
        [inputClasses enumerateObjectsUsingBlock:^(Class  _Nonnull inputClass, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            ExtendImplementationOfNonVoidMethodWithSingleArgument(inputClass, @selector(initWithFrame:), CGRect, UIView<UITextInputTraits> *, ^UIView<UITextInputTraits> *(UIView<UITextInputTraits> *selfObject, CGRect firstArgv, UIView<UITextInputTraits> *originReturnValue) {
                if ([selfObject isKindOfClass:NSClassFromString(@"TUIEmojiSearchTextField")]) {
                    ...
                    // 另外它属于 emoji 键盘内部的 TextFied，其 keyboardAppearance 应该由业务的 UITextField、UITextView 驱动，因此 ZGUI 也不应该去干预他
                    return originReturnValue;
                }
                if (ZGUICMIActivated) selfObject.keyboardAppearance = KeyboardAppearance;
            ...
            OverrideImplementation(inputClass, @selector(setKeyboardAppearance:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UIView<UITextInputTraits> *selfObject, UIKeyboardAppearance keyboardAppearance) {
                    ...
                    if (selfObject.qti_didInitialize && valueChanged) {
                        // 标志当前输入框希望有与配置表不一样的值，则在 ZGUITheme 发生变化时不要替它自动切换
                        if (ZGUICMIActivated && !selfObject.qti_setKeyboardAppearanceByZGUITheme) selfObject.zgui_hasCustomizedKeyboardAppearance = YES;
...
@implementation NSObject (ZGUITextInput_Private)

ZGUISynthesizeBOOLProperty(zgui_hasCustomizedKeyboardAppearance, setZgui_hasCustomizedKeyboardAppearance)

static char kAssociatedObjectKey_keyboardAppearance;
- (void)setZgui_keyboardAppearance:(UIKeyboardAppearance)zgui_keyboardAppearance {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_keyboardAppearance, @(zgui_keyboardAppearance), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.qti_setKeyboardAppearanceByZGUITheme = YES;
    ((UIView<UITextInputTraits> *)self).keyboardAppearance = zgui_keyboardAppearance;
    self.qti_setKeyboardAppearanceByZGUITheme = NO;
}

- (UIKeyboardAppearance)zgui_keyboardAppearance {
