...
@property(nonatomic, assign, readwrite) UITableViewStyle zgui_style;
@end

@implementation UITableView (ZGUI)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UITableView class], @selector(initWithFrame:style:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^UITableView *(UITableView *selfObject, CGRect firstArgv, UITableViewStyle secondArgv) {
                ...
                if (@available(iOS 13.0, *)) {
                    // iOS 13 zgui_style 的 getter 直接返回 tableView.style，所以这里不需要给 zgui_style 赋值
                } else {
                    selfObject.zgui_style = secondArgv;
                    if (secondArgv == ZGUITableViewStyleInsetGrouped) {
                ...
                // 而当使用 estimate 效果时，会导致 contentSize 之类的计算不准确，所以这里给一个途径让项目可以方便地控制 UITableView（不包含子类，例如 UIPickerTableView）的 estimatedRowHeight 效果的开关，至于 ZGUITableView 会在自己内部 init 时调用
                // https://github.com/Tencent/QMUI_iOS/issues/313
                if (ZGUICMIActivated && [NSStringFromClass(selfObject.class) isEqualToString:@"UITableView"]) {
                    [selfObject _zgui_configEstimatedRowHeight];
                }
        ...
        OverrideImplementation([UITableView class], @selector(scrollToRowAtIndexPath:atScrollPosition:animated:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject, NSIndexPath *indexPath, UITableViewScrollPosition scrollPosition, BOOL animated) {
                ...
                if (!isIndexPathLegal) {
//                    ZGUILogWarn(@"UITableView (ZGUI)", @"%@ - target indexPath : %@ ，不合法的indexPath。\n%@", selfObject, indexPath, [NSThread callStackSymbols]);
...
- (void)zgui_scrollToRowAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(UITableViewScrollPosition)scrollPosition animated:(BOOL)animated {
    if (!indexPath) {
        return;
    }
...
//        ZGUILogWarn(@"UITableView (ZGUI)", @"%@ - target indexPath : %@ ，不合法的indexPath。\n%@", self, indexPath, [NSThread callStackSymbols]);
        ...
        [self zgui_scrollToRowAtIndexPath:indexPath atScrollPosition:scrollPosition animated:animated];
    }
}

- (void)zgui_styledAsZGUITableView {
    
    if (!ZGUICMIActivated) return;
    
    [self _zgui_configEstimatedRowHeight];
    
    self.backgroundColor = PreferredValueForTableViewStyle(self.zgui_style, TableViewBackgroundColor, TableViewGroupedBackgroundColor, TableViewInsetGroupedBackgroundColor);
    self.separatorColor = PreferredValueForTableViewStyle(self.zgui_style, TableViewSeparatorColor, TableViewGroupedSeparatorColor, TableViewInsetGroupedSeparatorColor);
    
    // 去掉空白的cell
    if (self.zgui_style == UITableViewStylePlain) {
    ...
    self.zgui_insetGroupedCornerRadius = TableViewInsetGroupedCornerRadius;
    self.zgui_insetGroupedHorizontalInset = TableViewInsetGroupedHorizontalInset;
}

- (void)_zgui_configEstimatedRowHeight {
...
- (NSIndexPath *)zgui_indexPathForRowAtView:(UIView *)view {
    ...
    return [self zgui_indexPathForRowAtView:view.superview];
}

- (NSInteger)zgui_indexForSectionHeaderAtView:(UIView *)view {
...
- (NSArray<NSNumber *> *)zgui_indexForVisibleSectionHeaders {
    ...
    for (NSInteger i = 0; i < visibleSections.count; i++) {
        ...
        if ([self zgui_isHeaderVisibleForSection:section]) {
...
- (NSInteger)zgui_indexOfPinnedSectionHeader {
    NSArray<NSNumber *> *visibleSectionIndex = [self zgui_indexForVisibleSectionHeaders];
    for (NSInteger i = 0; i < visibleSectionIndex.count; i++) {
        NSInteger section = visibleSectionIndex[i].integerValue;
        if ([self zgui_isHeaderPinnedForSection:section]) {
            return section;
        } else {
            continue;
        }
...
- (BOOL)zgui_isHeaderPinnedForSection:(NSInteger)section {
    if (self.zgui_style != UITableViewStylePlain) return NO;
    ...
    BOOL isSectionScrollIntoContentInsetTop = self.contentOffset.y + self.zgui_contentInset.top > CGRectGetMinY(rectForSection);// 表示这个 section 已经往上滚动，超过 contentInset.top 那条线了
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.zgui_contentInset.top <= CGRectGetMaxY(rectForSection) - CGRectGetHeight(rectForHeader);// 表示这个 section 还没被完全滚走
    BOOL isPinned = isSectionScrollIntoContentInsetTop && isSectionStayInContentInsetTop;
    return isPinned;
}

- (BOOL)zgui_isHeaderVisibleForSection:(NSInteger)section {
    if (self.zgui_style != UITableViewStylePlain) return NO;
    if (section >= [self numberOfSections]) return NO;
    ...
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.zgui_contentInset.top < CGRectGetMaxY(rectForSection);// 表示这个 section 还没被完全滚走
    BOOL isVisible = isSectionScrollIntoBounds && isSectionStayInContentInsetTop;
    return isVisible;
}

- (ZGUITableViewCellPosition)zgui_positionForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSInteger numberOfRowsInSection = [self.dataSource tableView:self numberOfRowsInSection:indexPath.section];
    if (numberOfRowsInSection == 1) {
        return ZGUITableViewCellPositionSingleInSection;
    }
    if (indexPath.row == 0) {
        return ZGUITableViewCellPositionFirstInSection;
    }
    if (indexPath.row == numberOfRowsInSection - 1) {
        return ZGUITableViewCellPositionLastInSection;
    }
    return ZGUITableViewCellPositionMiddleInSection;
}

- (BOOL)zgui_cellVisibleAtIndexPath:(NSIndexPath *)indexPath {
    NSArray<NSIndexPath *> *visibleCellIndexPaths = self.indexPathsForVisibleRows;
...
- (void)zgui_scrollToRowFittingOffsetY:(CGFloat)offsetY atIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    [self alertEstimatedHeightUsageIfDetected];
    ...
    if (canScrollRowToTop) {
        ...
        [self zgui_scrollToBottomAnimated:animated];
    }
}

- (CGFloat)zgui_validContentWidth {
    return CGRectGetWidth(self.bounds) - UIEdgeInsetsGetHorizontalValue(self.zgui_safeAreaInsets) - (self.zgui_style == ZGUITableViewStyleInsetGrouped ? self.zgui_insetGroupedHorizontalInset * 2 : 0);
}

- (CGSize)zgui_realContentSize {
    [self alertEstimatedHeightUsageIfDetected];
...
- (BOOL)zgui_canScroll {
    ...
    if ([self.tableHeaderView isKindOfClass:[UISearchBar class]]) {
        BOOL canScroll = self.zgui_realContentSize.height + UIEdgeInsetsGetVerticalValue(self.zgui_contentInset) > CGRectGetHeight(self.bounds);
        return canScroll;
    } else {
        return [super zgui_canScroll];
    }
}
...
- (void)alertEstimatedHeightUsageIfDetected {
    ...
    if (usingEstimatedRowHeight || usingEstimatedSectionHeaderHeight || usingEstimatedSectionFooterHeight) {
        [self ZGUISymbolicUsingTableViewEstimatedHeightMakeWarning];
    }
}

- (void)ZGUISymbolicUsingTableViewEstimatedHeightMakeWarning {
//    ZGUILog(@"UITableView (ZGUI)", @"当开启了 UITableView 的 estimatedRow(SectionHeader / SectionFooter)Height 功能后，不应该手动修改 contentOffset 和 contentSize，也会影响 contentSize、sizeThatFits:、rectForXxx 等方法的计算，请注意确认当前是否存在不合理的业务代码。可添加 '%@' 的 Symbolic Breakpoint 以捕捉此类信息\n%@", NSStringFromSelector(_cmd), [NSThread callStackSymbols]);
}

- (void)zgui_performBatchUpdates:(void (NS_NOESCAPE ^ _Nullable)(void))updates completion:(void (^ _Nullable)(BOOL finished))completion {
    if (@available(iOS 11.0, *)) {
        ...
        if (!updates && completion) {
            ...
            [self zgui_performSelector:NSSelectorFromString([NSString stringWithFormat:@"_%@BatchUpdates:%@:", @"perform", @"completion"]) withArguments:&updates, &completion, nil];
        }
    }
}
...
@interface UITableViewCell (ZGUI_Private)

@property(nonatomic, assign, readwrite) ZGUITableViewCellPosition zgui_cellPosition;
@end

const UITableViewStyle ZGUITableViewStyleInsetGrouped = UITableViewStyleGrouped + 1;

@implementation UITableView (ZGUI_InsetGrouped)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UITableView class], NSSelectorFromString(@"_configureCellForDisplay:forIndexPath:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject, UITableViewCell *cell, NSIndexPath *indexPath) {
                ...
                // UITableViewCell(ZGUI) 内会根据 cellPosition 调整 separator 的布局，所以先在这里赋值以供那边使用
                ZGUITableViewCellPosition position = [selfObject zgui_positionForRowAtIndexPath:indexPath];
                cell.zgui_cellPosition = position;
                
                if (selfObject.zgui_style == ZGUITableViewStyleInsetGrouped) {
                    ZGUICornerMask mask = ZGUILayerAllCorner;
                    CGFloat cornerRadius = selfObject.zgui_insetGroupedCornerRadius;
                    switch (position) {
                        case ZGUITableViewCellPositionFirstInSection:
                            mask = ZGUILayerMinXMinYCorner|ZGUILayerMaxXMinYCorner;
                            break;
                        case ZGUITableViewCellPositionLastInSection:
                            mask = ZGUILayerMinXMaxYCorner|ZGUILayerMaxXMaxYCorner;
                            break;
                        case ZGUITableViewCellPositionMiddleInSection:
                        case ZGUITableViewCellPositionNone:
                            cornerRadius = 0;
                            break;
                        default:
                            break;
                    ...
                    if (@available(iOS 13.0, *)) {
                        ...
                        cell.layer.zgui_maskedCorners = mask;
        ...
        if (@available(iOS 13.0, *)) {
            OverrideImplementation([UITableView class], @selector(layoutMargins), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^UIEdgeInsets(UITableView *selfObject) {
                    ...
                    if (selfObject.zgui_style == ZGUITableViewStyleInsetGrouped) {
                        result.left = selfObject.zgui_safeAreaInsets.left + selfObject.zgui_insetGroupedHorizontalInset;
                        result.right = selfObject.zgui_safeAreaInsets.right + selfObject.zgui_insetGroupedHorizontalInset;
                    }
...
- (void)setZgui_style:(UITableViewStyle)zgui_style {
    if (@available(iOS 13.0, *)) {
    } else {
        objc_setAssociatedObject(self, &kAssociatedObjectKey_style, @(zgui_style), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}

- (UITableViewStyle)zgui_style {
    if (@available(iOS 13.0, *)) {
        return self.style;
    }
...
- (void)setZgui_insetGroupedCornerRadius:(CGFloat)zgui_insetGroupedCornerRadius {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_insetGroupedCornerRadius, @(zgui_insetGroupedCornerRadius), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.zgui_style == ZGUITableViewStyleInsetGrouped && self.indexPathsForVisibleRows.count) {
        [self reloadData];
    }
}

- (CGFloat)zgui_insetGroupedCornerRadius {
    ...
    return associatedValue.zgui_CGFloatValue;
}

static char kAssociatedObjectKey_insetGroupedHorizontalInset;
- (void)setZgui_insetGroupedHorizontalInset:(CGFloat)zgui_insetGroupedHorizontalInset {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_insetGroupedHorizontalInset, @(zgui_insetGroupedHorizontalInset), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.zgui_style == ZGUITableViewStyleInsetGrouped && self.indexPathsForVisibleRows.count) {
        [self reloadData];
    }
}

- (CGFloat)zgui_insetGroupedHorizontalInset {
