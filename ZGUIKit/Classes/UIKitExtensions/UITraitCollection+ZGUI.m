...
@implementation UITraitCollection (ZGUI)

static NSHashTable *_eventObservers;
static NSString * const kZGUIUserInterfaceStyleWillChangeSelectorsKey = @"zgui_userInterfaceStyleWillChangeObserver";

+ (void)zgui_addUserInterfaceStyleWillChangeObserver:(id)observer selector:(SEL)aSelector {
    if (@available(iOS 13.0, *)) {
        @synchronized (self) {
            [UITraitCollection _zgui_overrideTraitCollectionMethodIfNeeded];
            if (!_eventObservers) {
                _eventObservers = [NSHashTable weakObjectsHashTable];
            }
            NSMutableSet *selectors = [observer zgui_getBoundObjectForKey:kZGUIUserInterfaceStyleWillChangeSelectorsKey];
            if (!selectors) {
                selectors = [NSMutableSet set];
                [observer zgui_bindObject:selectors forKey:kZGUIUserInterfaceStyleWillChangeSelectorsKey];
            }
...
+ (void)_zgui_notifyUserInterfaceStyleWillChangeEvents:(UITraitCollection *)traitCollection {
...
zgui_getTraitCollection(UIWindow *targetWindow, id (*originalIMP)(id, SEL), SEL originCMD) {
#ifndef DEBUG
    // 非 DEBUG 无需处理 Main Thread Checker，直接用 originalIMP 取得 traitCollection
    return originalIMP(targetWindow, originCMD);
#endif
    // 以下代码只会在 DEBUG 生效，主要是屏蔽 Main Thread Checker 对 ZGUI swizzle traitCollection 的检测
    ...
    // Main Thread Checker 的原理是在启动的时候替换相关方法的实现为 Main Thread Checker 自身的 trampoline，触发相关方法时会先在 trampoline 中实现线程检测逻辑并告警，所以这里唯一可行的屏蔽方法就是获取原始的 IMP 实现，并直接调用从而绕过 Main Thread Checker, 由于 ZGUI 在 +load 到时候已经晚于这个时机，无法获取原始的实现方法，观察发现 -[UIWindow traitCollection] 内部会调用 -[UIWindow _updateWindowTraitsAndNotify:]，因此可以借助该方法回溯到 traitCollection 的真实地址。
    ...
    if (directTraitCollectionIMP) {
        ...
        dispatch_once(&onceToken, ^{
            if (zgui_exists_dyld_image("libMainThreadChecker.dylib")) {
        ...
        // 如果存在 Main Thread Checker，下面这行调用将会命中 _updateWindowTraitsAndNotify: 的 swizzle，并拿到 directTraitCollectionIMP，下一次 zgui_getTraitCollection 会直接调用 directTraitCollectionIMP 从而避开检测。
...
+ (void)_zgui_overrideTraitCollectionMethodIfNeeded {
    if (@available(iOS 13.0, *)) {
        [ZGUIHelper executeBlock:^{
            static BOOL _isOverridedMethodProcessing = NO;
            static UIUserInterfaceStyle zgui_lastNotifiedUserInterfaceStyle;
            zgui_lastNotifiedUserInterfaceStyle = [UITraitCollection currentTraitCollection].userInterfaceStyle;
            OverrideImplementation([UIWindow class] , @selector(traitCollection), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^UITraitCollection *(UIWindow *selfObject) {
                    ...
                    UITraitCollection *traitCollection = zgui_getTraitCollection(selfObject, originSelectorIMP, originCMD);
                    ...
                    if (selfObject.windowScene && !snapshotFinishedOnBackground) {
                        ...
                        if (selfObject == firstValidatedWindow) {
                            if (zgui_lastNotifiedUserInterfaceStyle != traitCollection.userInterfaceStyle) {
                                zgui_lastNotifiedUserInterfaceStyle = traitCollection.userInterfaceStyle;
                                [self _zgui_notifyUserInterfaceStyleWillChangeEvents:traitCollection];
                            }
