...
@implementation NSString (ZGUI)

- (NSArray<NSString *> *)zgui_toArray {
    if (!self.length) {
        return nil;
    }
...
- (NSArray<NSString *> *)zgui_toTrimmedArray {
    return [[self zgui_toArray] zgui_filterWithBlock:^BOOL(NSString *item) {
...
- (NSString *)zgui_trimAllWhiteSpace {
    return [self stringByReplacingOccurrencesOfString:@"\\s" withString:@"" options:NSRegularExpressionSearch range:NSMakeRange(0, self.length)];
}

- (NSString *)zgui_trimLineBreakCharacter {
...
- (NSString *)zgui_stringByEncodingUserInputQuery {
    return [self stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet zgui_URLUserInputQueryAllowedCharacterSet]];
}

- (NSString *)zgui_capitalizedString {
    if (self.length)
...
- (NSString *)zgui_removeMagicalChar {
    if (self.length == 0) {
        return self;
    }
...
- (NSUInteger)zgui_lengthWhenCountingNonASCIICharacterAsTwo {
    NSUInteger length = 0;
...
- (NSString *)zgui_substringAvoidBreakingUpCharacterSequencesFromIndex:(NSUInteger)index {
    return [self zgui_substringAvoidBreakingUpCharacterSequencesFromIndex:index lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)zgui_substringAvoidBreakingUpCharacterSequencesToIndex:(NSUInteger)index {
    return [self zgui_substringAvoidBreakingUpCharacterSequencesToIndex:index lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)zgui_substringAvoidBreakingUpCharacterSequencesWithRange:(NSRange)range {
    return [self zgui_substringAvoidBreakingUpCharacterSequencesWithRange:range lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)zgui_stringByRemoveLastCharacter {
    return [self zgui_stringByRemoveCharacterAtIndex:self.length - 1];
}
...
@implementation NSString (ZGUI_StringFormat)
...
+ (instancetype)zgui_stringWithCGFloat:(CGFloat)floatValue {
    return [NSString zgui_stringWithCGFloat:floatValue decimal:2];
}
