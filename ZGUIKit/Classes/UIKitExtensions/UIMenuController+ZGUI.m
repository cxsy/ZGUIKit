 ...
 */
//
//  UIMenuController+ZGUI.m
//  ZGUIKit
//
//  Created by 陈志宏 on 2019/7/21.
//

#import "UIMenuController+ZGUI.h"
#import "ZGUICore.h"

@interface UIMenuController ()

@property(nonatomic, assign) NSInteger zgui_originWindowLevel;
@property(nonatomic, assign) BOOL zgui_windowLevelChanged;

@end

@implementation UIMenuController (ZGUI)

ZGUISynthesizeNSIntegerProperty(zgui_originWindowLevel, setZgui_originWindowLevel);
ZGUISynthesizeBOOLProperty(zgui_windowLevelChanged, setZgui_windowLevelChanged);
...
- (void)handleMenuWillShowNotification:(NSNotification *)notification {
    ...
    if (window && targetWindow && ![ZGUIHelper isKeyboardVisible]) {
//        ZGUILog(NSStringFromClass(self.class), @"show menu - cur window level = %@, origin window level = %@ target window level = %@", @(window.windowLevel), @(self.zgui_originWindowLevel), @(targetWindow.windowLevel));
        self.zgui_windowLevelChanged = YES;
        self.zgui_originWindowLevel = window.windowLevel;
...
- (void)handleMenuWillHideNotification:(NSNotification *)notification {
    UIWindow *window = [self menuControllerWindow];
    if (window && self.zgui_windowLevelChanged) {
//        ZGUILog(NSStringFromClass(self.class), @"hide menu - cur window level = %@, origin window level = %@", @(window.windowLevel), @(self.zgui_originWindowLevel));
        window.windowLevel = self.zgui_originWindowLevel;
        self.zgui_originWindowLevel = 0;
        self.zgui_windowLevelChanged = NO;
    }
}
