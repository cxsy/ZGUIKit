...
typedef NS_OPTIONS (NSUInteger, ZGUICornerMask) {
    ZGUILayerMinXMinYCorner = 1U << 0,
    ZGUILayerMaxXMinYCorner = 1U << 1,
    ZGUILayerMinXMaxYCorner = 1U << 2,
    ZGUILayerMaxXMaxYCorner = 1U << 3,
    ZGUILayerAllCorner = ZGUILayerMinXMinYCorner|ZGUILayerMaxXMinYCorner|ZGUILayerMinXMaxYCorner|ZGUILayerMaxXMaxYCorner,
};

@interface CALayer (ZGUI)

/// 是否为某个 UIView 自带的 layer
@property(nonatomic, assign, readonly) BOOL zgui_isRootLayerOfView;

/// 暂停/恢复当前 layer 上的所有动画
@property(nonatomic, assign) BOOL zgui_pause;

/**
 *  设置四个角是否支持圆角的，iOS11 及以上会调用系统的接口，否则 ZGUI 额外实现
 *  @warning 如果对应的 layer 有圆角，则请使用 ZGUIBorder，否则系统的 border 会被 clip 掉
 *  @warning 使用 zgui 方法，则超出 layer 范围内的内容都会被 clip 掉，系统的则不会
 *  @warning 如果使用这个接口设置圆角，那么需要获取圆角的值需要用 zgui_originCornerRadius，否则 iOS 11 以下获取到的都是 0
 */
@property(nonatomic, assign) ZGUICornerMask zgui_maskedCorners;

/// iOS11 以下 layer 自身的 cornerRadius 一直都是 0，圆角的是通过 mask 做的，zgui_originCornerRadius 保存了当前的圆角
@property(nonatomic, assign) CGFloat zgui_originCornerRadius;
 ...
 * @note 如果该 layer 的任何属性修改都不需要动画，也可使用 zgui_removeDefaultAnimations。
 */
...
@interface CALayer (ZGUI_DynamicColor)

/// 如果 layer 的 backgroundColor、borderColor、shadowColor 是使用 dynamic color（UIDynamicProviderColor、ZGUIThemeColor 等）生成的，则调用这个方法可以重新设置一遍这些属性，从而更新颜色
/// iOS 13 系统设置里的界面样式变化（Dark Mode），以及 ZGUIThemeManager 触发的主题变化，都会自动调用 layer 的这个方法，业务无需关心。
- (void)zgui_setNeedsUpdateDynamicStyle NS_REQUIRES_SUPER;
@end
