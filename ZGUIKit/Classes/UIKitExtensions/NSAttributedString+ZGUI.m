...
//
//  NSAttributedString+ZGUI.m
//  zgui
//
//  Created by ZGUI Team on 16/9/23.
//

#import "NSAttributedString+ZGUI.h"
#import "ZGUICore.h"
#import "NSString+ZGUI.h"

@implementation NSAttributedString (ZGUI)
...
- (NSUInteger)zgui_lengthWhenCountingNonASCIICharacterAsTwo {
    return self.string.zgui_lengthWhenCountingNonASCIICharacterAsTwo;
}

+ (instancetype)zgui_attributedStringWithImage:(UIImage *)image {
    return [self zgui_attributedStringWithImage:image baselineOffset:0 leftMargin:0 rightMargin:0];
}

+ (instancetype)zgui_attributedStringWithImage:(UIImage *)image baselineOffset:(CGFloat)offset leftMargin:(CGFloat)leftMargin rightMargin:(CGFloat)rightMargin {
    if (!image) {
        return nil;
    }
    ...
    if (leftMargin > 0) {
        [string insertAttributedString:[self zgui_attributedStringWithFixedSpace:leftMargin] atIndex:0];
    }
    if (rightMargin > 0) {
        [string appendAttributedString:[self zgui_attributedStringWithFixedSpace:rightMargin]];
    }
    return string;
}

+ (instancetype)zgui_attributedStringWithFixedSpace:(CGFloat)width {
    UIGraphicsBeginImageContext(CGSizeMake(width, 1));
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return [self zgui_attributedStringWithImage:image];
}

@end

@implementation NSMutableAttributedString (ZGUI)
