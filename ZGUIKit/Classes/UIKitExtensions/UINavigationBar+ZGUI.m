...
//
//  UINavigationBar+ZGUI.m
//  ZGUIKit
//
//  Created by ZGUI Team on 2018/O/8.
//

#import "UINavigationBar+ZGUI.h"
#import "ZGUICore.h"
#import "NSObject+ZGUI.h"

@implementation UINavigationBar (ZGUI)
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (@available(iOS 11.0, *)) {
            ...
            ExtendImplementationOfVoidMethodWithoutArguments([UINavigationBar class], NSSelectorFromString(@"_updateBackgroundView"), ^(UINavigationBar *selfObject) {
                ...
                UIImageView *shadowImageView = selfObject.zgui_shadowImageView;
...
- (__kindof UIView *)zgui_backgroundContentView {
    if (@available(iOS 13, *)) {
        return [self.zgui_backgroundView zgui_valueForKey:@"_colorAndImageView1"];
    } else {
        UIImageView *imageView = [self.zgui_backgroundView zgui_valueForKey:@"_backgroundImageView"];
        UIVisualEffectView *visualEffectView = [self.zgui_backgroundView zgui_valueForKey:@"_backgroundEffectView"];
        UIView *customView = [self.zgui_backgroundView zgui_valueForKey:@"_customBackgroundView"];
