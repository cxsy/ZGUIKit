...
//  zgui
//
//  Created by ZGUI Team on 15/3/29.
//
...
@interface ZGUIConfiguration ()

@property(nonatomic, strong) UITabBarAppearance *tabBarAppearance API_AVAILABLE(ios(13.0));
@end

@implementation UIViewController (ZGUIConfiguration)

- (NSArray <UIViewController *>*)zgui_existingViewControllersOfClasses:(NSArray<Class<UIAppearanceContainer>> *)classes {
    NSMutableSet *viewControllers = [NSMutableSet set];
    if (self.presentedViewController) {
        [viewControllers addObjectsFromArray:[self.presentedViewController zgui_existingViewControllersOfClasses:classes]];
    }
    if ([self isKindOfClass:UINavigationController.class]) {
        [viewControllers addObjectsFromArray:[((UINavigationController *)self).visibleViewController zgui_existingViewControllersOfClasses:classes]];
    }
    if ([self isKindOfClass:UITabBarController.class]) {
        [viewControllers addObjectsFromArray:[((UITabBarController *)self).selectedViewController zgui_existingViewControllersOfClasses:classes]];
    }
...
@implementation ZGUIConfiguration

+ (instancetype)sharedInstance {
    static dispatch_once_t pred;
    static ZGUIConfiguration *sharedInstance;
    dispatch_once(&pred, ^{
        sharedInstance = [[ZGUIConfiguration alloc] init];
    });
...
static BOOL ZGUI_hasAppliedInitialTemplate;
- (void)applyInitialTemplate {
    if (ZGUI_hasAppliedInitialTemplate) {
        return;
    }
    ...
    Protocol *protocol = @protocol(BDECUIConfigurationTemplateProtocol);
    classref_t *classesref = nil;
    Class *classes = nil;
    int numberOfClasses = zgui_getProjectClassList(&classesref);
    ...
    for (NSInteger i = 0; i < numberOfClasses; i++) {
        ...
        if ([NSStringFromClass(class) containsString:@"UIConfigurationTemplate"] && [class conformsToProtocol:protocol]) {
            if ([class instancesRespondToSelector:@selector(shouldApplyTemplateAutomatically)]) {
                id<BDECUIConfigurationTemplateProtocol> template = [[class alloc] init];
                if ([template shouldApplyTemplateAutomatically]) {
                    ZGUI_hasAppliedInitialTemplate = YES;
...
- (void)initDefaultConfiguration {
    ...
    self.navBarCloseButtonImage = [UIImage zgui_imageWithShape:ZGUIImageShapeNavClose size:CGSizeMake(16, 16) tintColor:self.navBarTintColor];
    ...
    self.navBarAccessoryViewTypeDisclosureIndicatorImage = [[UIImage zgui_imageWithShape:ZGUIImageShapeTriangle size:CGSizeMake(8, 5) tintColor:self.navBarTitleColor] zgui_imageWithOrientation:UIImageOrientationDown];
    ...
    self.windowLevelZGUIAlertView = UIWindowLevelAlert - 4.0;
    self.windowLevelZGUIConsole = 1;
    
    #pragma mark - ZGUILog
    self.shouldPrintDefaultLog = YES;
    self.shouldPrintInfoLog = YES;
    self.shouldPrintWarnLog = YES;
    self.shouldPrintZGUIWarnLogToConsole = IS_DEBUG;
    
    #pragma mark - ZGUIBadge
    self.badgeOffset = ZGUIBadgeInvalidateOffset;
    self.badgeOffsetLandscape = ZGUIBadgeInvalidateOffset;
    self.updatesIndicatorOffset = ZGUIBadgeInvalidateOffset;
    self.updatesIndicatorOffsetLandscape = ZGUIBadgeInvalidateOffset;
...
- (void)setNavBarBarTintColor:(UIColor *)navBarBarTintColor {
    ...
    UINavigationBar.zgui_appearanceConfigured.barTintColor = _navBarBarTintColor;
    [self.appearanceUpdatingNavigationControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController,NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)configureNavBarShadowImage {
    ...
    if (shadowImage || self.navBarShadowImageColor) {
        if (shadowImage) {
            if (self.navBarShadowImageColor && shadowImage.renderingMode != UIImageRenderingModeAlwaysOriginal) {
                shadowImage = [shadowImage zgui_imageWithTintColor:self.navBarShadowImageColor];
            }
        } else {
            shadowImage = [UIImage zgui_imageWithColor:self.navBarShadowImageColor size:CGSizeMake(4, PixelOne) cornerRadius:0];
        }
    ...
    UINavigationBar.zgui_appearanceConfigured.shadowImage = shadowImage;
    [self.appearanceUpdatingNavigationControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController,NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setNavBarStyle:(UIBarStyle)navBarStyle {
    ...
    UINavigationBar.zgui_appearanceConfigured.barStyle = navBarStyle;
    [self.appearanceUpdatingNavigationControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController,NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setNavBarBackgroundImage:(UIImage *)navBarBackgroundImage {
    ...
    [UINavigationBar.zgui_appearanceConfigured setBackgroundImage:_navBarBackgroundImage forBarMetrics:UIBarMetricsDefault];
    [self.appearanceUpdatingNavigationControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController,NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)updateNavigationBarTitleAttributesIfNeeded {
    NSMutableDictionary<NSAttributedStringKey, id> *titleTextAttributes = UINavigationBar.zgui_appearanceConfigured.titleTextAttributes.mutableCopy;
    ...
    UINavigationBar.zgui_appearanceConfigured.titleTextAttributes = titleTextAttributes;
    [self.appearanceUpdatingNavigationControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController,NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)updateNavigationBarLargeTitleTextAttributesIfNeeded {
    if (@available(iOS 11, *)) {
        ...
        UINavigationBar.zgui_appearanceConfigured.largeTitleTextAttributes = largeTitleTextAttributes;
        [self.appearanceUpdatingNavigationControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController,NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setNavBarBackIndicatorImage:(UIImage *)navBarBackIndicatorImage {
    ...
    if (navBarBackIndicatorImage && self.sizeNavBarBackIndicatorImageAutomatically) {
        ...
        if (!CGSizeEqualToSize(customBackIndicatorImageSize, systemBackIndicatorImageSize)) {
            ...
            _navBarBackIndicatorImage = [[_navBarBackIndicatorImage zgui_imageWithSpacingExtensionInsets:UIEdgeInsetsMake(imageExtensionVerticalFloat,
                                                                                                                          0,
    ...
    UINavigationBar *navBarAppearance = UINavigationBar.zgui_appearanceConfigured;
...
- (void)setToolBarStyle:(UIBarStyle)toolBarStyle {
    ...
    UIToolbar.zgui_appearanceConfigured.barStyle = toolBarStyle;
...
- (void)setToolBarBarTintColor:(UIColor *)toolBarBarTintColor {
    ...
    UIToolbar.zgui_appearanceConfigured.barTintColor = _toolBarBarTintColor;
    [self.appearanceUpdatingToolbarControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setToolBarBackgroundImage:(UIImage *)toolBarBackgroundImage {
    ...
    [UIToolbar.zgui_appearanceConfigured setBackgroundImage:_toolBarBackgroundImage forToolbarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
    [self.appearanceUpdatingToolbarControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setToolBarShadowImageColor:(UIColor *)toolBarShadowImageColor {
    ...
    UIImage *shadowImage = toolBarShadowImageColor ? [UIImage zgui_imageWithColor:_toolBarShadowImageColor size:CGSizeMake(1, PixelOne) cornerRadius:0] : nil;
    [UIToolbar.zgui_appearanceConfigured setShadowImage:shadowImage forToolbarPosition:UIBarPositionAny];
    [self.appearanceUpdatingToolbarControllers enumerateObjectsUsingBlock:^(UINavigationController * _Nonnull navigationController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)updateTabBarAppearance {
    if (@available(iOS 13.0, *)) {
        UITabBar.zgui_appearanceConfigured.standardAppearance = self.tabBarAppearance;
...
- (void)setTabBarBarTintColor:(UIColor *)tabBarBarTintColor {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        UITabBar.zgui_appearanceConfigured.barTintColor = _tabBarBarTintColor;
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTabBarStyle:(UIBarStyle)tabBarStyle {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        UITabBar.zgui_appearanceConfigured.barStyle = tabBarStyle;
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTabBarBackgroundImage:(UIImage *)tabBarBackgroundImage {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        UITabBar.zgui_appearanceConfigured.backgroundImage = tabBarBackgroundImage;
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTabBarShadowImageColor:(UIColor *)tabBarShadowImageColor {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        UIImage *shadowImage = [UIImage zgui_imageWithColor:_tabBarShadowImageColor size:CGSizeMake(1, PixelOne) cornerRadius:0];
        [UITabBar.zgui_appearanceConfigured setShadowImage:shadowImage];
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTabBarItemTitleFont:(UIFont *)tabBarItemTitleFont {
    ...
    if (@available(iOS 13.0, *)) {
        [self.tabBarAppearance zgui_applyItemAppearanceWithBlock:^(UITabBarItemAppearance * _Nonnull itemAppearance) {
        ...
        NSMutableDictionary<NSString *, id> *textAttributes = [[NSMutableDictionary alloc] initWithDictionary:[UITabBarItem.zgui_appearanceConfigured titleTextAttributesForState:UIControlStateNormal]];
        if (_tabBarItemTitleFont) {
            textAttributes[NSFontAttributeName] = _tabBarItemTitleFont;
        }
        [UITabBarItem.zgui_appearanceConfigured setTitleTextAttributes:textAttributes forState:UIControlStateNormal];
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTabBarItemTitleFontSelected:(UIFont *)tabBarItemTitleFontSelected {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        NSMutableDictionary<NSString *, id> *textAttributes = [[NSMutableDictionary alloc] initWithDictionary:[UITabBarItem.zgui_appearanceConfigured titleTextAttributesForState:UIControlStateSelected]];
        if (tabBarItemTitleFontSelected) {
            textAttributes[NSFontAttributeName] = tabBarItemTitleFontSelected;
        }
        [UITabBarItem.zgui_appearanceConfigured setTitleTextAttributes:textAttributes forState:UIControlStateSelected];
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
            [tabBarController.tabBar.items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTabBarItemImageColor:(UIColor *)tabBarItemImageColor {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        UITabBar.zgui_appearanceConfigured.unselectedItemTintColor = tabBarItemImageColor;
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
//        UITabBar.zgui_appearanceConfigured.tintColor = tabBarItemImageColorSelected;
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setStatusbarStyleLightInitially:(BOOL)statusbarStyleLightInitially {
    ...
    [[ZGUIHelper visibleViewController] setNeedsStatusBarAppearanceUpdate];
}
...
- (NSArray <UITabBarController *>*)appearanceUpdatingTabBarControllers {
    ...
    classes = [classes zgui_filterWithBlock:^BOOL(Class<UIAppearanceContainer>  _Nonnull item) {
...
- (NSArray <UIViewController *>*)appearanceUpdatingViewControllersOfClasses:(NSArray<Class<UIAppearanceContainer>> *)classes {
    ...
    [UIApplication.sharedApplication.windows enumerateObjectsUsingBlock:^(__kindof UIWindow * _Nonnull window, NSUInteger idx, BOOL * _Nonnull stop) {
        if (window.rootViewController) {
            [viewControllers addObjectsFromArray:[window.rootViewController zgui_existingViewControllersOfClasses:classes]];
        }
    }];
...
@implementation UINavigationBar (ZGUIConfiguration)

+ (instancetype)zgui_appearanceConfigured {
    if (ZGUICMIActivated && NavBarContainerClasses) {
...
@implementation UITabBar (ZGUIConfiguration)

+ (instancetype)zgui_appearanceConfigured {
    if (ZGUICMIActivated && TabBarContainerClasses) {
...
@implementation UIToolbar (ZGUIConfiguration)

+ (instancetype)zgui_appearanceConfigured {
    if (ZGUICMIActivated && ToolBarContainerClasses) {
...
@implementation UITabBarItem (ZGUIConfiguration)
