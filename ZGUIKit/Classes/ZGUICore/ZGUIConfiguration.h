...
@protocol BDECUIConfigurationTemplateProtocol <NSObject>
...
@optional
/// 当返回 YES 时，启动 App 的时候 ZGUIConfiguration 会自动应用这份配置表。但启动 App 时自动应用的配置表最多只允许一份，如果有多份则其他的会被忽略
/// ZGUIConfiguration automatically applies this template on launch when set to YES. Since only one copy of configuration template is allowed when the app launches, you'll have to call `applyConfigurationTemplate` manually if you have more than one configuration templates. 
- (BOOL)shouldApplyTemplateAutomatically;

@end

/**
 *  维护项目全局 UI 配置的单例，通过业务项目自己的 BDECUIConfigurationTemplate 来为这个单例赋值，而业务代码里则通过 ZGUIConfigurationMacros.h 文件里的宏来使用这些值。
 *  A singleton that contains various UI configurations. Use `BDECUIConfigurationTemplate` to set values; Use macros in `ZGUIConfigurationMacros.h` to get values.
 */
@interface ZGUIConfiguration : NSObject
...
@property(nonatomic, assign) CGFloat            windowLevelZGUIAlertView;
@property(nonatomic, assign) CGFloat            windowLevelZGUIConsole;
...
@property(nonatomic, assign) BOOL               shouldPrintZGUIWarnLogToConsole;
...
@interface UINavigationBar (ZGUIConfiguration)
...
@interface UITabBar (ZGUIConfiguration)
...
@interface UIToolbar (ZGUIConfiguration)
...
@interface UITabBarItem (ZGUIConfiguration)
