...
//  zgui
//
//  Created by ZGUI Team on 14/10/25.
//
...
const CGPoint ZGUIBadgeInvalidateOffset = {-1000, -1000};
NSString *const kZGUIResourcesBundleName = @"ZGUIResources";

@implementation ZGUIHelper (Bundle)

+ (UIImage *)imageWithName:(NSString *)name {
    static NSBundle *resourceBundle = nil;
    if (!resourceBundle) {
        NSBundle *mainBundle = [NSBundle bundleForClass:self];
        NSString *resourcePath = [mainBundle pathForResource:kZGUIResourcesBundleName ofType:@"bundle"];
...
@implementation ZGUIHelper (DynamicType)
...
+ (CGFloat)heightForDynamicTypeCell:(NSArray *)heights {
    NSNumber *index = [ZGUIHelper preferredContentSizeLevel];
    return [((NSNumber *)[heights objectAtIndex:[index intValue]]) zgui_CGFloatValue];
}
@end

@implementation ZGUIHelper (Keyboard)

ZGUISynthesizeBOOLProperty(keyboardVisible, setKeyboardVisible)
ZGUISynthesizeCGFloatProperty(lastKeyboardHeight, setLastKeyboardHeight)

- (void)handleKeyboardWillShow:(NSNotification *)notification {
    self.keyboardVisible = YES;
    self.lastKeyboardHeight = [ZGUIHelper keyboardHeightWithNotification:notification];
}
...
+ (BOOL)isKeyboardVisible {
    BOOL visible = [ZGUIHelper sharedInstance].keyboardVisible;
    return visible;
}

+ (CGFloat)lastKeyboardHeightInApplicationWindowWhenVisible {
    return [ZGUIHelper sharedInstance].lastKeyboardHeight;
}
...
+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [ZGUIHelper keyboardHeightWithNotification:notification inView:nil];
}
...
+ (UIViewAnimationOptions)keyboardAnimationOptionsWithNotification:(NSNotification *)notification {
    UIViewAnimationOptions options = [ZGUIHelper keyboardAnimationCurveWithNotification:notification]<<16;
    return options;
}
...
@implementation ZGUIHelper (AudioSession)
...
@implementation ZGUIHelper (UIGraphic)
...
@implementation ZGUIHelper (Device)
        ...
        if (isNotchedScreen < 0) {
            if (@available(iOS 12.0, *)) {
                /*
                 ...
                 2. 与触发 [ZGUIHelper isNotchedScreen] 方法时的进程有关，例如 https://github.com/Tencent/QMUI_iOS/issues/482#issuecomment-456051738 里提到的 [NSObject performSelectorOnMainThread:withObject:waitUntilDone:NO] 就会导致较多的异常。
                ...
                [[UIScreen mainScreen] zgui_performSelector:peripheryInsetsSelector withPrimitiveReturnValue:&peripheryInsets];
                ...
                isNotchedScreen = [ZGUIHelper is58InchScreen] ? 1 : 0;
            }
        }
    } else {
        isNotchedScreen = 0;
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        ...
        is65InchScreen = (DEVICE_WIDTH == self.screenSizeFor65Inch.width && DEVICE_HEIGHT == self.screenSizeFor65Inch.height && ([[ZGUIHelper deviceModel] isEqualToString:@"iPhone11,4"] || [[ZGUIHelper deviceModel] isEqualToString:@"iPhone11,6"] || [[ZGUIHelper deviceModel] isEqualToString:@"iPhone12,5"])) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreenAndiPhone12 {
    if (is61InchScreenAndiPhone12 < 0) {
        is61InchScreenAndiPhone12 = (DEVICE_WIDTH == self.screenSizeFor61InchAndiPhone12.width && DEVICE_HEIGHT == self.screenSizeFor61InchAndiPhone12.height && ([[ZGUIHelper deviceModel] isEqualToString:@"iPhone13,2"] || [[ZGUIHelper deviceModel] isEqualToString:@"iPhone13,3"])) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreen {
    if (is61InchScreen < 0) {
        is61InchScreen = (DEVICE_WIDTH == self.screenSizeFor61Inch.width && DEVICE_HEIGHT == self.screenSizeFor61Inch.height && ([[ZGUIHelper deviceModel] isEqualToString:@"iPhone11,8"] || [[ZGUIHelper deviceModel] isEqualToString:@"iPhone12,1"])) ? 1 : 0;
    }
...
+ (CGFloat)preferredLayoutAsSimilarScreenWidthForIPad {
    if (preferredLayoutWidth < 0) {
        ...
        for (NSInteger i = 0; i < widths.count; i++) {
            if (windowWidth <= widths[i].zgui_CGFloatValue) {
                preferredLayoutWidth = widths[i].zgui_CGFloatValue;
                continue;
            }
        }
    }
...
+ (UIEdgeInsets)safeAreaInsetsForDeviceWithNotch {
    ...
    NSString *deviceKey = [ZGUIHelper deviceModel];
    if (!dict[deviceKey]) {
        deviceKey = @"iPhone12,5";// 默认按 iPhone 11 Pro Max
    }
    if ([ZGUIHelper isZoomedMode]) {
...
+ (BOOL)isHighPerformanceDevice {
    if (isHighPerformanceDevice < 0) {
        NSString *model = [ZGUIHelper deviceModel];
        NSString *identifier = [model zgui_stringMatchedByPattern:@"\\d+"];
...
@implementation ZGUIHelper (UIApplication)
...
@implementation ZGUIHelper (Animation)
...
@implementation ZGUIHelper (SystemVersion)
...
+ (BOOL)isCurrentSystemAtLeastVersion:(NSString *)targetVersion {
    return [ZGUIHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedSame || [ZGUIHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedDescending;
}

+ (BOOL)isCurrentSystemLowerThanVersion:(NSString *)targetVersion {
    return [ZGUIHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedAscending;
}

@end

@implementation ZGUIHelper

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [ZGUIHelper sharedInstance];// 确保内部的变量、notification 都正确配置
    });
}

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static ZGUIHelper *instance = nil;
    dispatch_once(&onceToken,^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:instance selector:@selector(handleAppSizeWillChange:) name:ZGUIAppSizeWillChangeNotification object:nil];
...
- (void)dealloc {
    // ZGUIHelper 若干个分类里有用到消息监听，所以在 dealloc 的时候注销一下
