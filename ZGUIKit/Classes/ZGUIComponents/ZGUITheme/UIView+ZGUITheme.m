...
#import "UITextInputTraits+ZGUI.h"

@implementation UIView (ZGUITheme)

ZGUISynthesizeIdCopyProperty(zgui_themeDidChangeBlock, setZgui_themeDidChangeBlock)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UIView class], @selector(setHidden:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIView *selfObject, BOOL firstArgv) {
                ...
                if (valueChanged) {
                    // UIView.zgui_currentThemeIdentifier 只是为了实现判断当前的 theme 是否有发生变化，所以可以构造成一个 string，但怎么避免每次 hidden 切换时都要遍历所有的 subviews？
                    [selfObject _zgui_themeDidChangeByManager:nil identifier:nil theme:nil shouldEnumeratorSubviews:YES];
                }
            };
        });
        ...
        [classes enumerateObjectsUsingBlock:^(Class  _Nonnull class, NSUInteger idx, BOOL * _Nonnull stop) {
            ExtendImplementationOfVoidMethodWithoutArguments(class, @selector(didMoveToWindow), ^(UIView *selfObject) {
                ...
                if (selfObject.window) {
                    [selfObject _zgui_themeDidChangeByManager:nil identifier:nil theme:nil shouldEnumeratorSubviews:NO];
                }
            });
        }];
    });
...
- (void)zgui_registerThemeColorProperties:(NSArray<NSString *> *)getters {
...
- (void)zgui_unregisterThemeColorProperties:(NSArray<NSString *> *)getters {
    if (!self.zguiTheme_themeColorProperties) return;
    
    [getters enumerateObjectsUsingBlock:^(NSString * _Nonnull getterString, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.zguiTheme_themeColorProperties removeObjectForKey:getterString];
    }];
}

- (void)zgui_themeDidChangeByManager:(ZGUIThemeManager *)manager identifier:(__kindof NSObject<NSCopying> *)identifier theme:(__kindof NSObject *)theme {
    if (![self _zgui_visible]) return;
    
    // 常见的 view 在 ZGUIThemePrivate 里注册了 getter，在这里被调用
    [self.zguiTheme_themeColorProperties enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull getterString, NSString * _Nonnull setterString, BOOL * _Nonnull stop) {
        ...
        if (getter == @selector(tintColor)) {
            if (!self.zgui_tintColorCustomized) return;
        }
        ...
        if (!value) return;
        BOOL isValidatedColor = [value isKindOfClass:ZGUIThemeColor.class] && (!manager || [((ZGUIThemeColor *)value).managerName isEqual:manager.name]);
        BOOL isValidatedImage = [value isKindOfClass:ZGUIThemeImage.class] && (!manager || [((ZGUIThemeImage *)value).managerName isEqual:manager.name]);
        BOOL isValidatedEffect = [value isKindOfClass:ZGUIThemeVisualEffect.class] && (!manager || [((ZGUIThemeVisualEffect *)value).managerName isEqual:manager.name]);
        ...
        if (isOtherObject || isValidatedColor || isValidatedImage || isValidatedEffect) {
            ...
            // 修复 iOS 12 及以下版本，ZGUIThemeImage 在搭配 resizable 使用的情况下可能无法跟随主题刷新的 bug
            // https://github.com/Tencent/QMUI_iOS/issues/971
            if (@available(iOS 13.0, *)) {
            } else {
                if (isValidatedImage) {
                    ZGUIThemeImage *image = (ZGUIThemeImage *)value;
                    if (image.zgui_resizable) {
                        value = image.copy;
                    }
                }
            }
    ...
    // 特殊的 view 特殊处理
    // iOS 10-11 里当 UILabel.attributedText 的文字颜色都相同时，也无法使用 setNeedsDisplay 刷新样式，但只要某个 range 颜色不同就没问题，iOS 9、12-13 也没问题，这个通过 UILabel (ZGUIThemeCompatibility) 兼容。
    // iOS 9-13，当 UITextField 没有聚焦时，不需要调用 setNeedsDisplay 系统都可以自动更新文字样式，但聚焦时调用 setNeedsDisplay 也无法更新样式，这里依赖了 UITextField (ZGUIThemeCompatibility) 对 setNeedsDisplay 做的兼容实现了更新
    // 注意，iOS 11 及以下的 UITextView 直接调用 setNeedsDisplay 是无法刷新文字样式的，这里依赖了 UITextView (ZGUIThemeCompatibility) 里通过 swizzle 实现了兼容，iOS 12 及以上没问题。
    ...
    // 输入框、搜索框的键盘跟随主题变化
    if (ZGUICMIActivated && [self conformsToProtocol:@protocol(UITextInputTraits)]) {
        NSObject<UITextInputTraits> *input = (NSObject<UITextInputTraits> *)self;
        if ([input respondsToSelector:@selector(keyboardAppearance)]) {
            if (input.keyboardAppearance != KeyboardAppearance && !input.zgui_hasCustomizedKeyboardAppearance) {
                input.zgui_keyboardAppearance = KeyboardAppearance;
            }
        }
    }
    
    /** 这里去掉动画有 2 个原因：
     1. iOS 13 进入后台时会对 currentTraitCollection.userInterfaceStyle 做一次取反进行截图，以便在后台切换 Drak/Light 后能够更新 app 多任务缩略图，ZGUI 响应了这个操作去调整取反后的 layer 的颜色，而在对 layer 设置属性的时候，如果包含了动画会导致截图不到最终的状态，这样会导致在后台切换 Drak/Light 后多任务缩略图无法及时更新。
     2. 对于 UIView 层，修改 backgroundColor 默认是没有动画的，而 CALayer 修改 backgroundColor 会有隐式动画，这里为了在响应主题变化时颜色同步更新，统一把 CALayer 的动画去掉
     */
    [CALayer zgui_performWithoutAnimation:^{
        [self.layer zgui_setNeedsUpdateDynamicStyle];
    }];
    
    if (self.zgui_themeDidChangeBlock) {
        self.zgui_themeDidChangeBlock();
    }
}
...
ZGUISynthesizeIdStrongProperty(zguiTheme_themeColorProperties, setZguiTheme_themeColorProperties)
...
- (void)_zgui_themeDidChangeByManager:(ZGUIThemeManager *)manager identifier:(__kindof NSObject<NSCopying> *)identifier theme:(__kindof NSObject *)theme shouldEnumeratorSubviews:(BOOL)shouldEnumeratorSubviews {
    [self zgui_themeDidChangeByManager:manager identifier:identifier theme:theme];
    if (shouldEnumeratorSubviews) {
        [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull subview, NSUInteger idx, BOOL * _Nonnull stop) {
            [subview _zgui_themeDidChangeByManager:manager identifier:identifier theme:theme shouldEnumeratorSubviews:YES];
        }];
    }
}
