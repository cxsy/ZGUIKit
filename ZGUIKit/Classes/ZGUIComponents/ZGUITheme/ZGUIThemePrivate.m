...
#import "UITabBarItem+ZGUI.h"

@interface ZGUIThemePropertiesRegister : NSObject

@end

@implementation ZGUIThemePropertiesRegister
            ...
            ({
                ...
                if (!classRegisters) {
                    classRegisters = @{
                        ...
                        NSStringFromClass(UITableViewCell.class):            @[NSStringFromSelector(@selector(zgui_selectedBackgroundColor)),],
                        ...
                        NSStringFromClass(UITabBar.class):                   ({
                            ...
                            if (@available(iOS 13.0, *)) {
                                // iOS 13 在 UITabBar (ZGUI) 里对所有旧版接口都映射到 standardAppearance，所以重新设置一次 standardAppearance 就可以更新所有样式
                        ...
                        // 以下的 class 的更新依赖于 UIView (ZGUITheme) 内的 setNeedsDisplay，这里不专门调用 setter
                ...
                [classRegisters enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull classString, NSArray<NSString *> * _Nonnull getters, BOOL * _Nonnull stop) {
                    if ([selfObject isKindOfClass:NSClassFromString(classString)]) {
                        [selfObject zgui_registerThemeColorProperties:getters];
                    }
                }];
            });
...
@implementation UIView (ZGUIThemeCompatibility)
    ...
    dispatch_once(&onceToken, ^{
        if (@available(iOS 13.0, *)) {
            ...
            OverrideImplementation([UIView class], @selector(setTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UIView *selfObject, UIColor *tintColor) {
                    ...
                    if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.tintColor) tintColor = tintColor.copy;
        ...
        OverrideImplementation([UIView class], @selector(setBackgroundColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^void(UIView *selfObject, UIColor *color) {
                ...
                if (selfObject.backgroundColor.zgui_isZGUIDynamicColor || color.zgui_isZGUIDynamicColor) {
                    // -[UIView setBackgroundColor:] 会同步修改 layer 的 backgroundColor，但它内部又有一个判断条件即：如果参入传入的 color.CGColor 和当前的 self.layr.backgroundColor 一样，就不会重新设置，而如果 layer.backgroundColor 如果关联了 ZGUI 的动态色，忽略这个设置，就会导致前后不一致的问题，这里要强制把 layer.backgroundColor 清空，让每次都调用 -[CALayer setBackgroundColor:] 方法
        ...
        // iOS 12 及以下的版本，[UIView setBackgroundColor:] 并不会保存传进来的 color，所以要自己用个变量保存起来，不然 ZGUIThemeColor 对象就会被丢弃
        if (@available(iOS 13.0, *)) {
        } else {
            ExtendImplementationOfVoidMethodWithSingleArgument([UIView class], @selector(setBackgroundColor:), UIColor *, ^(UIView *selfObject, UIColor *color) {
                [selfObject zgui_bindObject:color forKey:@"UIView(ZGUIThemeCompatibility).backgroundColor"];
            });
            ExtendImplementationOfNonVoidMethodWithoutArguments([UIView class], @selector(backgroundColor), UIColor *, ^UIColor *(UIView *selfObject, UIColor *originReturnValue) {
                UIColor *color = [selfObject zgui_getBoundObjectForKey:@"UIView(ZGUIThemeCompatibility).backgroundColor"];
...
@implementation UISwitch (ZGUIThemeCompatibility)
        ...
        if (@available(iOS 13.0, *)) {
            OverrideImplementation([UISwitch class], @selector(setOnTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UISwitch *selfObject, UIColor *tintColor) {
                    ...
                    if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.onTintColor) tintColor = tintColor.copy;
            ...
            OverrideImplementation([UISwitch class], @selector(setThumbTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UISwitch *selfObject, UIColor *tintColor) {
                    ...
                    if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.thumbTintColor) tintColor = tintColor.copy;
...
@implementation UISlider (ZGUIThemeCompatibility)
    ...
    dispatch_once(&onceToken, ^{
        OverrideImplementation([UISlider class], @selector(setMinimumTrackTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UISlider *selfObject, UIColor *tintColor) {
                ...
                if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.minimumTrackTintColor) tintColor = tintColor.copy;
        ...
        OverrideImplementation([UISlider class], @selector(setMaximumTrackTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UISlider *selfObject, UIColor *tintColor) {
                ...
                if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.maximumTrackTintColor) tintColor = tintColor.copy;
...
@implementation UIProgressView (ZGUIThemeCompatibility)
    ...
    dispatch_once(&onceToken, ^{
        OverrideImplementation([UIProgressView class], @selector(setProgressTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIProgressView *selfObject, UIColor *tintColor) {
                ...
                if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.progressTintColor) tintColor = tintColor.copy;
        ...
        OverrideImplementation([UIProgressView class], @selector(setTrackTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIProgressView *selfObject, UIColor *tintColor) {
                ...
                if (tintColor.zgui_isZGUIDynamicColor && tintColor == selfObject.trackTintColor) tintColor = tintColor.copy;
...
@implementation UITabBarItem (ZGUIThemeCompatibility)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // UITabBarItem.image 会一直保存原始的 image（例如 ZGUIThemeImage），但 selectedImage 只会返回 rawImage，这导致了将一个 ZGUIThemeImage 设置给 selectedImage 后，主题切换后 selectedImage 无法刷新（因为 UITabBarItem 并没有保存它，保存的是它的 rawImage），所以这里自己保存 image 的引用。
        ...
        OverrideImplementation([UITabBarItem class], @selector(setSelectedImage:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITabBarItem *selfObject, UIImage *selectedImage) {
                ...
                [selfObject zgui_bindObject:selectedImage forKey:@"UITabBarItem(ZGUIThemeCompatibility).selectedImage"];
            };
        });
        ...
        OverrideImplementation([UITabBarItem class], @selector(selectedImage), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^UIImage *(UITabBarItem *selfObject) {
                ...
                UIImage *selectedImage = [selfObject zgui_getBoundObjectForKey:@"UITabBarItem(ZGUIThemeCompatibility).selectedImage"];
                if (selectedImage) {
...
@implementation UITableViewCell (ZGUIThemeCompatibility)
        ...
        if (@available(iOS 13.0, *)) {
            ...
            OverrideImplementation([UITableViewCell class], @selector(setBackgroundColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UITableViewCell *selfObject, UIColor *backgroundColor) {
                     if (backgroundColor.zgui_isZGUIDynamicColor && backgroundColor == selfObject.backgroundColor) backgroundColor = backgroundColor.copy;
...
@implementation UIVisualEffectView (ZGUIThemeCompatibility)
    ...
    dispatch_once(&onceToken, ^{
        OverrideImplementation([UIVisualEffectView class], @selector(setEffect:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIVisualEffectView *selfObject, UIVisualEffect *effect) {
                ...
                if (effect.zgui_isDynamicEffect && effect == selfObject.effect) effect = effect.copy;
...
@implementation UILabel (ZGUIThemeCompatibility)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // iOS 10-11 里，UILabel.attributedText 如果整个字符串都是同个颜色，则调用 -[UILabel setNeedsDisplay] 无法刷新文字样式，但如果字符串中存在不同 range 有不同颜色，就可以刷新。iOS 9、12-13 都没这个问题，所以这里做了兼容，给 UIView (ZGUITheme) 那边刷新 UILabel 用。
                ...
                return ^BOOL(UILabel *selfObject) {
                    ...
                    if (selfObject.attributedText) {
                        [selfObject.attributedText enumerateAttribute:NSForegroundColorAttributeName inRange:NSMakeRange(0, selfObject.attributedText.length) options:0 usingBlock:^(UIColor *color, NSRange range, BOOL * _Nonnull stop) {
                            if (color.zgui_isZGUIDynamicColor) {
...
@implementation UITextField (ZGUIThemeCompatibility)
    ...
    dispatch_once(&onceToken, ^{
        ...
        ExtendImplementationOfVoidMethodWithoutArguments([UITextField class], @selector(setNeedsDisplay), ^(UITextView *selfObject) {
            if (selfObject.isFirstResponder) {
                UIView *fieldEditor = [selfObject zgui_valueForKey:@"_fieldEditor"];
                if (fieldEditor) {
                    UIView *contentView = [fieldEditor zgui_valueForKey:@"_contentView"];
                    if (contentView) {
...
@implementation UITextView (ZGUIThemeCompatibility)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // UITextView 在 iOS 12 及以上重写了 -[UIView setNeedsDisplay]，在里面会去刷新文字样式，但 iOS 11 及以下没有重写，所以这里对此作了兼容，从而保证 ZGUITheme 那边遇到 UITextView 时能使用 setNeedsDisplay 刷新文字样式。至于实现思路是参考 iOS 13 系统原生实现。
        if (@available(iOS 12.0, *)) {
        } else {
            ExtendImplementationOfVoidMethodWithoutArguments([UITextView class], @selector(setNeedsDisplay), ^(UITextView *selfObject) {
                UIView *textContainerView = [selfObject zgui_valueForKey:@"_containerView"];
...
@implementation CALayer (ZGUIThemeCompatibility)

ZGUISynthesizeIdStrongProperty(qcl_originalBackgroundColor, setQcl_originalBackgroundColor)
ZGUISynthesizeIdStrongProperty(qcl_originalBorderColor, setQcl_originalBorderColor)
ZGUISynthesizeIdStrongProperty(qcl_originalShadowColor, setQcl_originalShadowColor)
    ...
    dispatch_once(&onceToken, ^{
        OverrideImplementation([CALayer class], @selector(setBackgroundColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(CALayer *selfObject, CGColorRef color) {
                ...
                // iOS 13 的 UIDynamicProviderColor，以及 ZGUIThemeColor 在获取 CGColor 时会将自身绑定到 CGColorRef 上，这里把原始的 color 重新获取出来存到 property 里，以备样式更新时调用
        ...
        // 如果是 ZGUIThemeManager 引发的主题变化，会在 theme 那边主动调用 zgui_setNeedsUpdateDynamicStyle，就不依赖这里
        if (@available(iOS 13.0, *)) {
            ExtendImplementationOfVoidMethodWithoutArguments([UIView class], @selector(layoutSubviews), ^(UIView *selfObject) {
                [selfObject.layer zgui_setNeedsUpdateDynamicStyle];
            });
        }
    });
}
...
- (void)zgui_setNeedsUpdateDynamicStyle {
    ...
    [self.sublayers enumerateObjectsUsingBlock:^(__kindof CALayer * _Nonnull sublayer, NSUInteger idx, BOOL * _Nonnull stop) {
        if (!sublayer.zgui_isRootLayerOfView) {// 如果是 UIView 的 rootLayer，它会依赖 UIView 树自己的 layoutSubviews 去逐个触发，不需要手动遍历到，这里只需要遍历那些额外添加到 layer 上的 sublayer 即可
            [sublayer zgui_setNeedsUpdateDynamicStyle];
        }
    }];
}
...
@property(nonatomic, readonly) NSMutableDictionary <NSString * ,NSInvocation *>*zguiTheme_invocations;

@end

@implementation UISearchBar (ZGUIThemeCompatibility)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UISearchBar class], @selector(setSearchFieldBackgroundImage:forState:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            ...
            return ^(UISearchBar *selfObject, UIImage *image, UIControlState state) {
                ...
                if (previousImage.zgui_isDynamicImage || image.zgui_isDynamicImage) {
                    ...
                    // 由于 ZGUIDynamicImage 随时可能发生图片的改变，这里要绕过这个判断：必须先清空一下 image，并马上调用 layoutIfNeeded 触发 -[UITextFieldBorderView setImage:] 使得 UITextFieldBorderView 内部的 image 清空，这样再设置新的才会生效。
                    originSelectorIMP(selfObject, originCMD, UIImage.new, state);
                    [selfObject.zgui_textField setNeedsLayout];
                    [selfObject.zgui_textField layoutIfNeeded];
                }
                ...
                selfObject.zguiTheme_invocations[invocationActionKey] = invocation;
            };
        });
        ...
        OverrideImplementation([UISearchBar class], @selector(setBarTintColor:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UISearchBar *selfObject, UIColor *barTintColor) {
                ...
                if (barTintColor.zgui_isZGUIDynamicColor && barTintColor == selfObject.barTintColor) barTintColor = barTintColor.copy;
...
- (void)_zgui_themeDidChangeByManager:(ZGUIThemeManager *)manager identifier:(__kindof NSObject<NSCopying> *)identifier theme:(__kindof NSObject *)theme shouldEnumeratorSubviews:(BOOL)shouldEnumeratorSubviews {
    [super _zgui_themeDidChangeByManager:manager identifier:identifier theme:theme shouldEnumeratorSubviews:shouldEnumeratorSubviews];
    [self zguiTheme_performUpdateInvocations];
}

- (void)zguiTheme_performUpdateInvocations {
    [[self.zguiTheme_invocations allValues] enumerateObjectsUsingBlock:^(NSInvocation * _Nonnull invocation, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSMutableDictionary *)zguiTheme_invocations {
    NSMutableDictionary *zguiTheme_invocations = objc_getAssociatedObject(self, _cmd);
    if (!zguiTheme_invocations) {
        zguiTheme_invocations = [NSMutableDictionary dictionary];
        objc_setAssociatedObject(self, _cmd, zguiTheme_invocations, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return zguiTheme_invocations;
}
